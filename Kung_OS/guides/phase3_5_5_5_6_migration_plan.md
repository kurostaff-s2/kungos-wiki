...
**Target Databases:**
- **PostgreSQL:** `KungOS_PG_One` (Phase 5, 5.6 — relational tables)
- **MongoDB:** `KungOS_Mongo_One` (Phase 3, 5.5 — document collections)
...
## Phase 5: Stock + PO + Vendors → PostgreSQL (`KungOS_PG_One`)
...
## Phase 5.6: Accounts Master Data → PostgreSQL (`KungOS_PG_One`)
...
**Note:** 
- `partners`, `banks`, `loans` are TARGET PostgreSQL tables in `KungOS_PG_One`, not MongoDB collections
...
Step 5: Phase 5.6 (Accounts) ← Create PG tables in KungOS_PG_One + initial data   ┘
```
...
### After Phase 5
- [ ] `KungOS_PG_One` database contains new tables
...
### After Phase 5.6
- [ ] `KungOS_PG_One` contains `acct_partners`, `acct_banks`, `acct_loans` tables
...
# For PostgreSQL tables (KungOS_PG_One): Drop tables and re-run migrations
